...
class SceneSegSemDataset(Dataset):
    ...
    def load_scenes_in_memory(self, label_property='label', f_properties=[], f_scales=[], save_cache=True):
        ...
        for i, file_path in enumerate(self.scene_files):
            ...
            if exists(KDTree_file):
                ...
                # read ply with data
                if dl > 0:
                    points, sub_labels = self.load_scene_file(sub_ply_file)
                else:
                ...
                points, labels = self.load_scene_file(file_path)
                ...
                if dl > 0:
                    ...
                    if labels is None:
                        ...
                        sub_points, sub_labels = sub_data
                        # sub_features *= np.array(f_scales, dtype=np.float32)
                        if save_cache:
                            write_ply(sub_ply_file,
                                    [sub_points, sub_labels.astype(np.int32)],
                                    ['x', 'y', 'z'] + [label_property])

                else:
                    sub_points, sub_features, sub_labels = (points, labels)
        ...
        if dl > 0 and self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.scene_files):
                ...
                if exists(proj_file):
                    ...
                    proj_inds = self.input_trees[i].query(points, return_distance=False)
                    print('holaa')
    ...
    def sample_input_center(self, center_noise=0.05):
        ...
        if self.data_sampler == 'regular':
            ...
            if self.data_sampler == 'c-random':
                ...
                cloud_ind = np.random.choice(len(self.input_labels))
                point_ind = np.random.choice(self.input_labels[cloud_ind].shape[0])
    ...
    def get_input_area(self, cloud_ind, center_point, only_inds=False):
        ...
        return input_inds, input_points, None, input_labels
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            in_features = np.hstack((np.copy(in_points[:, 2:]))).astype(np.float32).reshape(-1,1)
...
class SceneSegSemSampler(Sampler):
    """Sampler for SceneSegSemDataset"""

    def __init__(self, dataset: SceneSegSemDataset):
...
class SceneSegSemBatch:
    """Custom batch definition with memory pinning for SceneSegSemDataset"""
...
def SceneSegSemCollate(batch_data):
    return SceneSegSemBatch(batch_data)
