...
def cloud_segmentation_test(epoch, net, test_loader, cfg, test_data, device, saving_path=None, save_visu=False):
    ...
    if 'probs' not in test_data:
        test_data.probs = [torch.zeros(
            (l.shape[0], nc_model), device=device) for l in test_loader.dataset.input_labels]
    ...
    for step, batch in enumerate(test_loader):
        ...
        # print(batch.in_dict.lengths)
        # print(batch.in_dict['points'][1].shape)
    ...
    if available_labels:
        for c_i, sub_labels in enumerate(test_loader.dataset.input_labels):
            ...
            # Confs
            pred_values = np.array(cfg.data.pred_values, dtype=np.int32)
            sub_confs += [fast_confusion(sub_labels,
                                         all_sub_preds[c_i], pred_values).astype(np.int64)]
    ...
    for c_i, sub_preds in enumerate(all_sub_preds):
        ...
        preds = (sub_preds[test_loader.dataset.test_proj[c_i]]).astype(
            np.int32)
    ...
    if available_labels:
        ...
        for c_i, preds in enumerate(all_preds):
            ...
            # Confusion matrix
            pred_values = np.array(cfg.data.pred_values, dtype=np.int32)
            full_confs.append(fast_confusion(
                labels, preds, pred_values).astype(np.int64))
    ...
    if saving:
        ...
        for c_i, sub_preds in enumerate(all_sub_preds):
            ...
            if save_visu:
                ...
                sub_points = np.array(
                    test_loader.dataset.input_trees[c_i].data, copy=False)
                if test_loader.dataset.cylindric_input:
                    support_points = np.hstack(
                        (support_points, test_loader.dataset.input_z[i]))

                # We first save the subsampled version for visu
                if available_labels:
                    write_ply(sub_name,
                              [sub_points, sub_preds,
                                  all_sub_labels[c_i].astype(np.int32)],
                              ['x', 'y', 'z', 'preds', 'class'])
                else:
                    write_ply(sub_name,
                              [sub_points, sub_preds],
            ...
            if cfg.data.name == 'Lima3D':
                cloud_name = files[c_i].split('/')[-1]
                full_preds = all_preds[c_i]
                # Re-load full resolution points from original file
                full_points, _ = test_loader.dataset.load_scene_file(files[c_i])
                # print('DEBUG: cloud_name =', cloud_name)
                # print('DEBUG: full_points shape =', full_points.shape)
                # print('DEBUG: full_preds shape =', full_preds.shape)
                full_name = join(saving_path, cloud_name[:-4] + '.ply')
                write_ply(full_name,
                        [full_points, full_preds.astype(np.int32)],
                        ['x', 'y', 'z', 'class'])
...
def object_classification_test(epoch, net, test_loader, cfg, test_data, device, saving_path=None, save_visu=False):
    ...
    # Voting
    test_data.probs[obj_inds] = test_smooth * \
        test_data.probs[obj_inds] + (1-test_smooth) * probs
