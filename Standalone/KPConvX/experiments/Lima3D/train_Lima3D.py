...
from experiments.Lima3D.Lima3D_cities import Lima3D_cfg, Lima3DDataset
...
def my_config():
    ...
    cfg.model.in_sub_size = 0.06        # Adapt this with train.in_radius. Try to keep a ratio of ~50 (*0.67 if fps). If negative, and fps, it is stride
    cfg.model.in_sub_mode = 'grid'      # Mode for input subsampling
    cfg.model.radius_scaling = 2.2      # Increase conv radius by this much  
    cfg.train.class_w = [0.2, 0.6, 0.6, 3.0, 2.5]
    ...
    cfg.model.input_channels = 1    # This value has to be compatible with one of the dataset input features definition
    ...
    # cfg.model.neighbor_limits = [10, 12, 20, 20, 20]    # List for point_transformer
    ...
    cfg.train.data_sampler = 'c-random'     #  'random', 'A-random' or 'c-random' for class balanced random sampling

    # Input spheres radius. Adapt this with model.in_sub_size. Try to keep a ratio of ~50
    cfg.train.in_radius = 2.5               # If negative, =number of points per input. Use negative to compare models
    ...
    # Training length
    cfg.train.max_epoch = 400  # 100
    
    # Deformations
    cfg.train.deform_loss_factor = 0.1      # Reduce to reduce influence for deformation on overall features
    cfg.train.deform_lr_factor = 0.5        # Higher so that deformation are learned faster (especially if deform_loss_factor is low)
    ...
    # How do we sample the input elements (spheres or cubes)
    cfg.test.in_radius = 5.0                # For S3DIS 4 meters is very large, cover a whole part of the test set with full rooms
    cfg.test.data_sampler = 'regular'       # 'regular' to pick spheres regularly accross the data.

    cfg.test.max_steps_per_epoch = 2000       # Size of one validation epoch (should be small)
...
if __name__ == '__main__':
    ...
    if args.dataset_path is not None:
        cfg.data.update(Lima3D_cfg(cfg, dataset_path=args.dataset_path).data)
    else:
        cfg.data.update(Lima3D_cfg(cfg).data)
    ...
    training_dataset = Lima3DDataset(cfg,
                                    chosen_set='training',
                                    precompute_pyramid=True)
    underline('Loading validation dataset')
    test_dataset = Lima3DDataset(cfg,
