...
#
#   KPConvX project: test_Lima3D.py
#       > Test script for Lima3D experiments
#
...
# from experiments.Lima3D.Lima3D import Lima3D_cfg, Lima3DDataset
from experiments.Lima3D.Lima3D_cities import Lima3D_cfg, Lima3DDataset

from tasks.test import test_model


def profile_Lima3D(net, test_loader, cfg, on_gpu=False, get_flops=False, test_path=""):
...
def test_Lima3D_log(chosen_log, new_cfg, weight_path='', save_visu=False, profile=False):
    ...
    test_dataset = Lima3DDataset(new_cfg,
                                chosen_set='test',
    ...
    # Go
    if profile:
        profile_Lima3D(net, test_loader, new_cfg, get_flops=False, test_path=test_path)
    else:
        test_model(net, test_loader, new_cfg, on_gpu=True, save_visu=save_visu, test_path=test_path)
    return
...
if __name__ == '__main__':
    ...
    new_cfg.test.num_workers = 12

    # Test whole rooms
    new_cfg.test.in_radius = 10.0
    new_cfg.data.use_cubes = True
    new_cfg.data.cylindric_input = False

    # Only stop one test epoch when all rooms have been tested
    new_cfg.test.max_steps_per_epoch = 99999

    # Test 10 times []
    new_cfg.test.max_votes = 1
    ...
    test_Lima3D_log(log_dir, new_cfg, weight_path=weights, profile=args.profile)
